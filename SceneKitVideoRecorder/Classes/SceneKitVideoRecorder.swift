...
public class SceneKitVideoRecorder: NSObject, AVAudioRecorderDelegate {
  ...
  public func startWriting() {
    ...
    public func finishWriting(completionHandler: ((_ url: URL) -> Void)?) {
    ...
    writer.finishWriting(completionHandler: { [weak self] in
        ...
        if completionHandler != nil {
            if this.useAudio {
                this.mergeVideoAndAudio(videoUrl: this.options.videoOnlyUrl, audioUrl: this.options.audioOnlyUrl) {(result) in
                    if result {
  ...
  private func mergeVideoAndAudio(videoUrl:URL, audioUrl:URL, completion: @escaping (_ result: Bool) -> Void)
  {
    ...
    if aVideoAsset.tracks.count == 0 || aAudioAsset.tracks.count == 0 {
        completion(false)
        return
    }
